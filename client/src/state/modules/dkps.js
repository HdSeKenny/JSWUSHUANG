...
export const mutations = {
  ...
  SOCKET_auction_receive(state, val) {
    ...
    if (!isSamePayer) {
      ...
      Object.assign(previousPayerDkp, { payment: previousPayment, sum: previousSum })
    }
  },
...
export const actions = {
  fetchDKPData({ commit, state, rootState }) {
    ...
    const requestWrapper = () => (admins.includes(currentUser.role)) ?
      HttpRequest.get('/api/dkps') :
      HttpRequest.get(`/api/dkps/${currentUser.game_id}`)

    return requestWrapper().then((data) => {
  ...
  updateDKPInfo({ commit, state, rootState }, params) {
    ...
    return HttpRequest
      .post(`/api/dkps/${edittedDkp._id}/info`, {
        ...params,
  ...
  calculateManyInfo({ commit, rootState }, data) {
    ...
    return HttpRequest
      .post(`/api/dkps/some/update`, { newData, histories })
      .then((_data) => {
    ...
    return HttpRequest.post('/api/ocr', formData, false, {
      ...
      data.words_result.forEach(wr => {
        const wdkp = state.DKPData.find(dp => {
          ...
          return !INVALID_CHARACTERS.includes(readableWords)
            && readableWords && dkpChinese.includes(readableWords)
        })
        if (wdkp) {
          members.push({
